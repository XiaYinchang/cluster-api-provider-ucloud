...
REGISTRY ?= uhub.service.ucloud.cn/uk8s
STAGING_REGISTRY := uhub.service.ucloud.cn/uk8s
PROD_REGISTRY := uhub.service.ucloud.cn/uk8s
IMAGE_NAME ?= cluster-api-ucloud-controller
...
RELEASE_ALIAS_TAG=$(RELEASE_TAG)

.PHONY: release-alias-tag
release-alias-tag: # Adds the tag to the last build tag.
	docker tag $(CONTROLLER_IMG)-amd64:$(TAG) $(CONTROLLER_IMG):$(RELEASE_ALIAS_TAG)
...
create-cluster: $(CLUSTERCTL) $(KUSTOMIZE) $(ENVSUBST) ## Create a development Kubernetes cluster on UCLOUD in a KIND management cluster.
	...
	# Wait for CAPG pod
	kubectl wait --for=condition=Ready --timeout=5m -n capu-system pod -l control-plane=capu-controller-manager
